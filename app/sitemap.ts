...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const articleRoutes = getPublicArticles().map(({ slug, updatedAt }) => ({ url: `${base}/article/${slug}`, lastModified: new Date(updatedAt), changeFrequency: 'monthly' as const, priority: .7 }));
